...
void MainWindow::removeText(){
    ...
    if (selected.isEmpty()){
        QMessageBox::warning(this, "错误", "请先选中文字");
        return;
    }
    GTextItem* item = qgraphicsitem_cast<GTextItem*>(selected.at(0));
    QMessageBox isRemove{this};
    isRemove.setInformativeText("是否删除文字");
    isRemove.setStandardButtons(QMessageBox::Yes | QMessageBox::Cancel);
    isRemove.setDefaultButton(QMessageBox::Cancel);
    int isremove = isRemove.exec();
    switch (isremove) {
    case QMessageBox::Yes:
        smgr->removeText(item->name);
        break;
...
void MainWindow::Start_(){
    ...
    if(ui->isten->isChecked())
    {
        ...
        for(QStringList &li : namelist){
            smgr->changeAllText(li);
            smgr->saveImage(FolderName+"/"+li.at(0)+"."+ui->picFormat->currentText().toLower(), ui->picFormat->currentText().toUtf8().constData());
            ui->progressBar->setValue(v);
            v++;
        }
    }
